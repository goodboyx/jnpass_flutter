...
class Singo extends StatefulWidget {
  ...
  Singo(
      {Key? key, required this.bo_table, required this.wr_id})
      : super(key: key);

  @override
  singoState createState() => singoState();
}

class singoState extends State<Singo> {
  ...
  Future<void> uploadAction() async {
    ...
    final parameters = {"jwt_token":jwtToken, "ca_id":currentSelectedValue, "bs_reason" : await json.decode(json.encode(_controller.text))};
    JsonApi.postApi("rest/singo/${widget.bo_table}/${widget.wr_id}", parameters).then((value) {
      ApiResponse apiResponse = ApiResponse();
